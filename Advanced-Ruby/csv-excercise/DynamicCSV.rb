...
def create_class (filename)
  ...
  Object::const_set($class_name.to_sym, Class.new do
    (0...$method_names.size).to_a.each do |i|
      define_method("#{$method_names[i].intern}") do
        instance_variable_get("@#{$method_names[i]}")
      end
      define_method("#{$method_names[i].intern}=") do |value|
        instance_variable_set("@#{$method_names[i]}", value)
      end
    end
  end
  )
